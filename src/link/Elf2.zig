...
pub const Node = union(enum) {
    ...
    pub const NavMapIndex = enum(u32) {
        ...
        pub fn symbolIndex(nmi: NavMapIndex, elf: *const Elf) Symbol.Index {
    ...
    pub const UavMapIndex = enum(u32) {
        ...
        pub fn symbolIndex(umi: UavMapIndex, elf: *const Elf) Symbol.Index {
    ...
    pub const LazyMapRef = struct {
        ...
        pub fn Index(comptime kind: link.File.LazySymbol.Kind) type {
            return enum(u32) {
                ...
                pub fn symbolIndex(lmi: @This(), elf: *const Elf) Symbol.Index {
                    return lmi.ref().symbolIndex(elf);
                }
            };
        }
        ...
        pub fn symbolIndex(lmr: LazyMapRef, elf: *const Elf) Symbol.Index {
    ...
    switch (node) {
        ...
        .nav, .uav, .lazy_code, .lazy_const_data => {
            const si = switch (node) {
                ...
                inline .nav, .uav, .lazy_code, .lazy_const_data => |mi| mi.symbolIndex(elf),
            };
            switch (elf.shdrSlice()) {
                inline else => |shdr, class| {
                    const sym = @field(elf.symPtr(si), @tagName(class));
