...
pub fn main() !void {
    ...
    var options = options: {
        if (cli_args.len > 0 and std.mem.eql(u8, cli_args[0], "targets")) {
            try subcommands.targets.run();
            return;
        } else if (cli_args.len > 0 and std.mem.eql(u8, cli_args[0], "cvtres")) {
            const subcommand_args = cli_args[1..];
            ...
            const options = subcommands.cvtres.parseCli(allocator, subcommand_args, &cli_diagnostics) catch |err| switch (err) {
                error.ParseError => {
                    try error_handler.emitCliDiagnostics(allocator, subcommand_args, &cli_diagnostics);
                    std.process.exit(1);
                },
                else => |e| return e,
            };
            ...
            if (options.print_help_and_exit) {
                try subcommands.cvtres.writeUsage(stderr.writer(), "zig rc cvtres");
                return;
            }

            break :options options;
        } else if (cli_args.len > 0 and std.mem.eql(u8, cli_args[0], "windres")) {
            const subcommand_args = cli_args[1..];
            var cli_diagnostics = cli.Diagnostics.init(allocator);
            defer cli_diagnostics.deinit();
            ...
            if (options.print_help_and_exit) {
                try subcommands.windres.writeUsage(stderr.writer(), "zig windres");
                return;
            }
    ...
    for (options.additional_inputs.items) |additional_res| {
        const additional_file = openFileNotDir(std.fs.cwd(), additional_res, .{}) catch |err| {
            try error_handler.emitMessage(allocator, .err, "unable to read res file path '{s}': {s}", .{ additional_res, @errorName(err) });
            std.process.exit(1);
        };
        defer additional_file.close();

        const file_len = additional_file.getEndPos() catch |err| {
            try error_handler.emitMessage(allocator, .err, "unable to determine size of res file '{s}': {s}", .{ additional_res, @errorName(err) });
            std.process.exit(1);
        };
        ...
        cvtres.parseResInto(&resources, buffered_reader.reader(), .{ .max_size = file_len }) catch |err| {
            // TODO: Better errors
            try error_handler.emitMessage(allocator, .err, "unable to parse res file '{s}': {s}", .{ additional_res, @errorName(err) });
            std.process.exit(1);
        };
    }
