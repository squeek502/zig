...
pub const Walker = struct {
    ...
    pub fn next(self: *Walker) !?Walker.Entry {
        var selective_walker = SelectiveWalker{
        ...
        defer {
            self.* = .{
                .allocator = selective_walker.allocator,
                .name_buffer = selective_walker.name_buffer,
                .stack = selective_walker.stack,
            };
        }
        const entry = try selective_walker.next();
        if (entry != null and entry.?.kind == .directory) {
            try selective_walker.enter(entry.?);
        }
        return entry;
    }
