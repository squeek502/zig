...
pub fn main() !void {
    ...
    // getEnvVarOwned
    {
        ...
        try std.testing.expectEqualSlices(u8, "non-ascii", try std.process.getEnvVarOwned(arena, "КИРиллИЦА"));
        if (builtin.os.tag == .windows) {
            try std.testing.expectEqualSlices(u8, "non-ascii", try std.process.getEnvVarOwned(arena, "кирИЛЛица"));
        }
    ...
    // EnvMap
    {
        ...
        try std.testing.expectEqualSlices(u8, "non-ascii", env_map.get("КИРиллИЦА").?);
        if (builtin.os.tag == .windows) {
            try std.testing.expectEqualSlices(u8, "non-ascii", env_map.get("кирИЛЛица").?);
        }
